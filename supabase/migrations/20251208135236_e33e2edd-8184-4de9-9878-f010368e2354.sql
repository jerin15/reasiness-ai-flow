-- Update the designer UPDATE policy to allow moving mockup tasks
...
USING (
  ...
  AND (
    ...
    OR (status = 'production'::task_status AND came_from_designer_done = true)
    OR (completed_by_designer_id = auth.uid())
  )
)
WITH CHECK (
  has_role(auth.uid(), 'designer'::app_role) 
  AND (
    ...
    OR (completed_by_designer_id = auth.uid())
    OR (status = 'with_client'::task_status)
    OR (status = 'production'::task_status)
    OR (status = 'done'::task_status)
  )
);